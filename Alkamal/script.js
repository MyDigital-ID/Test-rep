...
// هل التطبيق شغال بالفعل كتطبيق مثبت (standalone)؟
function isRunningAsInstalledApp(){
  return window.matchMedia('(display-mode: standalone)').matches ||
         window.navigator.standalone === true; // iOS
}

// هل التطبيق اتثبت أو المستخدم قفل الرسالة قبل كده (بشكل دائم)؟
function installBannerShouldStayHidden(){
  return isRunningAsInstalledApp() ||
         localStorage.getItem('alkamal-installed') === '1' ||
         localStorage.getItem('alkamal-install-dismissed') === '1';
}

window.addEventListener('beforeinstallprompt', (e) => {
  e.preventDefault();
  deferredInstallPrompt = e;
  if(!installBannerShouldStayHidden()){
    document.getElementById('install-banner').hidden = false;
  }
});

// أهم إضافة: لما التثبيت يحصل فعليًا، اقفل الرسالة للأبد
window.addEventListener('appinstalled', () => {
  localStorage.setItem('alkamal-installed', '1');
...
document.getElementById('install-later').addEventListener('click', () => {
  alert('تم الضغط على لاحقًا'); // TEMP: سطر تجريبي، هنشيله بعد التأكد
  document.getElementById('install-banner').hidden = true;
  localStorage.setItem('alkamal-install-dismissed', '1');
});

// عند تحميل الصفحة: لو شغالة كتطبيق مثبت أصلاً، اتأكد إن الرسالة مخفية من الأول
if(installBannerShouldStayHidden()){
  document.getElementById('install-banner').hidden = true;
}
